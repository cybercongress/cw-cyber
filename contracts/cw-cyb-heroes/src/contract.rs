...
const CONTRACT_NAME: &str = "cw-heroes";
...
pub fn execute(
    ...
    match msg {
        ExecuteMsg::NewEntry {
            ...
            chain_id,
            particle,
        } => execute_create_new_item(deps, info, address, chain_id,particle),
        ExecuteMsg::UpdateEntry {
            id,
            address,
            chain_id,
            particle,
        } => execute_update_item(deps, info, id, address, chain_id, particle),
...
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        QueryMsg::GetItems { start_after, limit, id, owner } => {
            to_binary(&query_list(deps, start_after, limit, id, owner)?)
        }
    }
}