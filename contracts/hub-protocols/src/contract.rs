...
use crate::query::{execute_create_new_item, execute_delete_entry, execute_update_item, execute_update_owner, query_list};
use crate::state::{Config, CONFIG, ENTRY_SEQ};

//@TODO git version iteract
const CONTRACT_NAME: &str = "hub-protocols";
...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::NewEntry {
            data_type,
            particle,
        } => execute_create_new_item(deps, info, data_type, particle),
        ExecuteMsg::UpdateEntry {
            id,
            data_type,
            particle,
        } => execute_update_item(deps, info, id, data_type, particle),
