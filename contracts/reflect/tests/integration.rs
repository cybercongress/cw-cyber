...
fn proper_initialization() {
    ...
    let res: Response<CyberMsgWrapper> = instantiate(&mut deps, mock_env(), info, msg).unwrap();
...
fn reflect() {
    ...
    let _res: Response<CyberMsgWrapper> = instantiate(&mut deps, mock_env(), info, msg).unwrap();
    ...
    let res: Response<CyberMsgWrapper> = execute(&mut deps, mock_env(), info, msg).unwrap();
...
fn reflect_requires_owner() {
    ...
    let res: ContractResult<Response<CyberMsgWrapper>> = execute(&mut deps, mock_env(), info, msg);
...
fn reflect_subcall() {
    ...
    let info = mock_info("creator", &[]);
    let mut res: Response<CyberMsgWrapper> = execute(&mut deps, mock_env(), info, msg).unwrap();
